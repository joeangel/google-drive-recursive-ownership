            ...
            for child in children.get('items', []):
                ...
                if item['kind'] == 'drive#file':
                    if current_prefix[:len(minimum_prefix)] == minimum_prefix:
                        #print(u'File: {} ({}, {})'.format(item['title'].encode('utf-8').strip(), current_prefix.encode('utf-8').strip(), item['id']))
                        callback(service, item, current_prefix, **callback_args)
                    if item['mimeType'] == 'application/vnd.google-apps.folder':
                        #print(u'Folder: {} ({}, {})'.format(item['title'].encode('utf-8').strip(), current_prefix, item['id']))
